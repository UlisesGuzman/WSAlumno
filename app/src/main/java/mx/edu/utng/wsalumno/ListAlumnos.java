...
public class ListAlumnos extends ListActivity {
    ...
    public boolean onContextItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            case R.id.item_modificar:
                Alumno alumno = alumnos.get(posicionSeleccionado);
    ...
    public void onCreateContextMenu(ContextMenu menu, View v,
        ...
        idSeleccionado = (Integer) alumnos.get(info.position).getProperty(0);
            ...
            try {
                ...
                if (response != null) {
                    for (SoapObject objSoap : response) {
                        ...
                        alumno.setProperty(0, Integer.parseInt(objSoap.getProperty("cveUniversidad").toString()));
                        alumno.setProperty(1, objSoap.getProperty("matricula").toString());
                        alumno.setProperty(2, objSoap.getProperty("notas").toString());
                        alumno.setProperty(3, objSoap.getProperty("fechaAlta").toString());
                        alumno.setProperty(4, Boolean.parseBoolean(objSoap.getProperty("becado").toString()));
                        alumno.setProperty(5, objSoap.getProperty("grupo").toString());
                        alumno.setProperty(6, Integer.parseInt(objSoap.getProperty("generacion").toString()));
                        alumno.setProperty(7, objSoap.getProperty("nombre").toString());
                        alumnos.add(alumno);
                    }
                }
            ...
            if (result) {
                ...
                for (int i = 0; i < alumnos.size(); i++) {
                    datos[i] = alumnos.get(i).getProperty(0) + " - "
                            ...
                            + alumnos.get(i).getProperty(5)+ " - "
                            + alumnos.get(i).getProperty(6)+ " - "
                            + alumnos.get(i).getProperty(7);
                }
    ...
    private class TaskWSDelete extends AsyncTask<String, Integer, Boolean> {
        ...
        protected Boolean doInBackground(String... params) {
            ...
            SoapObject request = new SoapObject(NAMESPACE, METHOD_NAME);
            request.addProperty("cveUniversidad", idSeleccionado);
